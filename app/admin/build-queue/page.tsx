    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-10 text-center">
          <h2 className="text-2xl font-bold text-[#1C1C1C] mb-2">Build Queue Empty</h2>
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <Link href="/admin" className="text-amber-600 text-sm font-semibold hover:underline mb-4 inline-block print:hidden">← Back to Admin</Link>
            <h1 className="text-3xl font-bold text-[#1C1C1C]">Shop Build Queue</h1>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handlePrint} className="bg-[#1C1C1C] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-800 print:hidden shadow-sm">
              🖨️ Print Queue
            </button>
          </div>
        </div>
        ...
        {/* Print Layout */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 print:shadow-none print:border-0 print:bg-transparent">
          {data.itemMap.map((item, index) => (
            <div key={item.name} className={`p-6 ${index > 0 ? 'border-t border-gray-100 print:break-inside-avoid' : ''}`}>
              <div className="flex justify-between items-end mb-4 bg-gray-50 p-3 rounded-lg border border-gray-100">
                <h3 className="text-lg font-bold text-[#1C1C1C]">{item.name}</h3>
                <div className="text-xl font-black text-amber-700 bg-amber-100 px-3 py-1 rounded border border-amber-200">
                  {item.qty}
                </div>
              </div>
                    ...
                    {item.orders.map((order, i) => (
                      <tr key={i} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 text-gray-500 font-mono text-xs">
                          <Link href={`/admin/orders?id=${order.orderId}`} className="hover:text-amber-600 hover:underline">
                            {order.orderId.slice(-8).toUpperCase()}
                          </Link>
                        </td>
                        <td className="py-3 text-center font-bold text-gray-700">{order.qty}</td>
                        <td className="py-3 font-medium text-[#1C1C1C]">{order.customerName}</td>
